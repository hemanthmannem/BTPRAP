CLASS zhem_cl_data_builder DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zhem_cl_data_builder IMPLEMENTATION.
...
*        output =
  ...
  METHOD fill_master_data.
    data : lt_bp type table of ZHEM_TBL_BPA,
           lt_prod type table of ZHEM_TBL_PRODUCT.
   ...
   append value #(
    ...
    append value #(
     ...
     insert ZHEM_TBL_BPA from table @lt_bp.
     insert ZHEM_TBL_PRODUCT from table @lt_prod.
  ENDMETHOD.
  ...
  METHOD fill_transaction_data.
    data : o_rand type REF TO cl_abap_random_int,
           ...
           lv_ord_id type zhem_de_id,
           lt_so type table of ZHEM_TBL_SO_HDR,
           lt_so_i type table of ZHEM_TBL_SO_ITEM.
    ...
    select * from ZHEM_TBL_BPA into table @data(lt_bpa).
    select * from ZHEM_TBL_PRODUCT into table @data(lt_prod).
    ...
    insert ZHEM_TBL_SO_HDR from table @lt_so.
    insert ZHEM_TBL_SO_ITEM from table @lt_so_i.

  ENDMETHOD.

  METHOD flush.
    delete from : ZHEM_TBL_BPA, ZHEM_TBL_PRODUCT, ZHEM_TBL_SO_HDR, ZHEM_TBL_SO_ITEM.
  ENDMETHOD.
ENDCLASS.
